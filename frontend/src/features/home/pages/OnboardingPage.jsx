...
import { completeOnboarding, startOnboarding, submitOnboardingAnswers } from '../services/homeOnboardingApi'
    ...
    <section className={shellClass}>
      <header className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-slate-50/80 p-4">
        ...
        <nav className="flex flex-wrap gap-2" aria-label="Onboarding navigation tabs">
          ...
          <button type="button" className={secondaryButtonClass} onClick={onOpenGlobalTerminal}>Market & Portfolio</button>
          <button type="button" className={secondaryButtonClass} onClick={() => onOpenGuidedInvesting?.('overview')}>BCTC</button>
          <button type="button" className={secondaryButtonClass} onClick={onOpenProLab}>Simulation Lab</button>
        </nav>
        ...
        <div className="mt-4 rounded-xl border border-slate-200 bg-white p-4">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            <button type="button" onClick={handleComplete} disabled={loading || answeredCount < QUESTIONS.length} className={primaryButtonClass}>
              {loading ? 'Đang lưu hồ sơ...' : 'Hoàn tất onboarding'}
            </button>
